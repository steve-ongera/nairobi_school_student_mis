...
export default function ExamForm() {
  ...
  useEffect(() => {
    // Load terms and forms
    Promise.all([getTerms(), getForms()])
      .then(([t, f]) => {
        // Defensive checks to ensure we're setting arrays
        setTerms(Array.isArray(t.data) ? t.data : []);
        setForms(Array.isArray(f.data) ? f.data : []);
        setLoadingData(false);
      })
      .catch(err => {
        console.error("Failed to load terms and forms:", err);
        setTerms([]);
        setForms([]);
        setLoadingData(false);
        setError("Failed to load terms and forms. Please refresh the page.");
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (typeof d === "string") {
        ...
        const errorMessage = Object.values(d).flat().join(" | ");
        setError(errorMessage || "Failed to save exam.");
      } else {
        setError("Failed to save exam. Please check your connection and try again.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <>
        ...
        <div className="card">
          <div className="card-body text-center py-5">
            ...
            <p className="mt-3 text-muted">Loading exam data...</p>
          </div>
        </div>
      </>
    );
            ...
            <div className="row g-3">
              ...
              <div className="col-md-3">
                ...
                <input 
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, max_marks: parseInt(e.target.value) || 0 }))} 
                />
              </div>
                ...
                >
                  ...
                  {Array.isArray(terms) && terms.map((t) => (
                    <option key={t.id} value={t.id}>
                      {t.academic_year_display || t.name || `Term ${t.term_number}`} 
                      {t.term_number && ` – Term ${t.term_number}`}
                    </option>
                  ))}
                </select>
              </div>
                  ...
                  {Array.isArray(forms) && forms.length > 0 ? (
                    forms.map((fm) => (
                      <div key={fm.id} className="form-check">
                        ...
                        />
                        <label className="form-check-label" htmlFor={`form-${fm.id}`}>
                          {fm.name}
                        </label>
                      </div>
                    ))
                  ) : (
                    <p className="text-muted">No forms available. Please add forms first.</p>
                  )}
                </div>
              </div>
              ...
              {isEdit && (
                <div className="col-12">
                  <div className="form-check form-switch">
                    ...
                    />
                    <label className="form-check-label fw-600" htmlFor="isPublished">
